...
public class DeviceListActivity extends Activity {
    ...
    public static final String ALLOW_INSECURE_CONNECTION = "false";

    // Member fields
    private BluetoothAdapter mBtAdapter;
    private ArrayAdapter<String> mNewDevicesArrayAdapter;

    private boolean mAllowInsecureConnections = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize the button to toggle secure/insecure connections
        Button toggleButton = findViewById(R.id.button_toggle);
        toggleButton.setOnClickListener(v -> toggleInSecure());
    ...
    private void toggleInSecure() {
        if (D) Log.d(TAG, "toggleInSecure()");

        TextView inSecure = findViewById(R.id.title_secure);
        if (!mAllowInsecureConnections) {
            mAllowInsecureConnections = true;
            inSecure.setText("allow insecure connections");
        } else {
            mAllowInsecureConnections = false;
            inSecure.setText("secure connections only");
        }
    }
        ...
        public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3) {
            ...
            if (!info.equals(noDevicesPaired) && !info.equals(noDevicesFound)) {
                ...
                if (info.length() >= 17) {
                    ...
                    if (mAllowInsecureConnections) {
                        intent.putExtra(ALLOW_INSECURE_CONNECTION, "true");
                    } else {
                        intent.putExtra(ALLOW_INSECURE_CONNECTION, "false");
                    }
