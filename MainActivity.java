...
public class MainActivity extends AppCompatActivity {
    ...
    static final String Version = "4.14";
                ...
                result -> {
                    if (result.getResultCode() == Activity.RESULT_OK) {
                        ...
                        if (iniValues != null) {
                            ...
                            if (value != null)
                                try {
                                    i_Safety.dbl = parseDouble(value.replace(',', '.'));
                                } catch (NumberFormatException e) {
                                    if (DEBUG) Log.i(TAG, "NumberFormatException " + e);
    ...
    public boolean onPrepareOptionsMenu (Menu menu) {
        ...
        MenuItem itemMenuConnect = menu.getItem(MENU_CON);
        MenuItem itemMenuData = menu.getItem(MENU_CLT);
        MenuItem itemMenuComputing = menu.getItem(MENU_CMP);
        MenuItem itemMenuRecording = menu.getItem(MENU_REC);
        MenuItem itemMenuAbout = menu.getItem(MENU_ABT);

        SpannableString menu_title = new SpannableString("Enter initial values");
        menu_title.setSpan(new ForegroundColorSpan(Color.WHITE), 0, menu_title.length(), 0);

        itemMenuInitial.setTitle(menu_title);

        menu_title = SpannableString.valueOf("Reset dongle");
        itemMenuReset.setTitle(menu_title);

        if (isConnected)
            menu_title = SpannableString.valueOf("Disconnect dongle");
        else menu_title = SpannableString.valueOf("Connect dongle");
        itemMenuConnect.setTitle(menu_title);

        menu_title = SpannableString.valueOf("Start all");
        itemMenuAll.setTitle(menu_title);

        if (isCollecting)
            menu_title = SpannableString.valueOf("Stop data");
        else menu_title = SpannableString.valueOf("Start data");
        itemMenuData.setTitle(menu_title);

        if (isComputing)
            menu_title = SpannableString.valueOf("Stop computing");
        else menu_title = SpannableString.valueOf("Start computing");
        itemMenuComputing.setTitle(menu_title);

        if (isRecording)
            menu_title = SpannableString.valueOf("Stop recording");
        else menu_title = SpannableString.valueOf("Start recording");
        itemMenuRecording.setTitle(menu_title);

        menu_title = SpannableString.valueOf("About");
        itemMenuAbout.setTitle(menu_title);
    ...
    private void StoreOBD(String datetime) {
        if (fileOBD != null && fileOBD.exists()) {
            ...
            strArray.add(datetime + ";Version;" + Version.replace(".", "") + "\r\n");
